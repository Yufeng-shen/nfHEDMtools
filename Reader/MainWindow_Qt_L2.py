...
#                                         1.67946465e-03,  -8.97547675e-03,  -8.87805809e-04])
    ...
    def changeoffset(self,d):
        ...
        if d > 0:
            ...
            self.photo[2]=self._client.GetImg(self.omegid+self.omegaoffset+2+720,
                    self.y1,self.y2,self.x1,self.x2)
        else:
            for ii in [2,1]:
                self.photo[ii]=self.photo[ii-1]
            self.photo[0]=self._client.GetImg(self.omegid+self.omegaoffset+720,
                    self.y1,self.y2,self.x1,self.x2)
        self.on_draw()
    ...
    def enterPeakID(self):
        ...
        for ii in range(3):
            self.photo[ii]=self._client.GetImg(self.omegid+self.omegaoffset+ii+720,
                    self.y1,self.y2,self.x1,self.x2)
